...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers {
    self = [self init];
    if (self) {
        if ([XTool isArrayEmpty:viewControllers] == NO) {
...
- (void)updateFrame {
    ...
    [_scrollView setContentSize:CGSizeMake(_scrollView.frame.size.width * _scrollView.subviews.count, _scrollView.frame.size.height)];
    
    if ([XTool isArrayEmpty:self.childViewControllers] == YES) {
        return;
    }
    ...
    for (UIViewController *childViewController in self.childViewControllers) {
        if (childViewController.view.superview != nil) {
            ...
            [childViewController.view setFrame:CGRectMake(_scrollView.frame.size.width * index, 0, _scrollView.frame.size.width, _scrollView.frame.size.height)];
        }
    }
}
...
- (void)removeViewController:(UIViewController *)viewController {
    ...
    if ([XTool isArrayEmpty:self.childViewControllers] == NO) {
...
- (void)addChildViewControllers {
    ...
    if ([XTool isArrayEmpty:_viewControllers] == NO) {
...
- (void)addChildViewController:(UIViewController *)childViewController atIndex:(NSInteger)atIndex {
    ...
    if (childViewController.view.superview == nil) {
        [childViewController.view setFrame:CGRectMake(_scrollView.frame.size.width * atIndex, 0, _scrollView.frame.size.width, _scrollView.frame.size.height)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewDidScroll:)]) {
        [_delegate xPageScrollViewDidScroll:scrollView];
    }
}

- (void)scrollViewDidZoom:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewDidZoom:)]) {
        [_delegate xPageScrollViewDidZoom:scrollView];
    }
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewWillBeginDragging:)]) {
        [_delegate xPageScrollViewWillBeginDragging:scrollView];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewWillEndDragging:withVelocity:targetContentOffset:)]) {
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewDidEndDragging:willDecelerate:)]) {
        [_delegate xPageScrollViewDidEndDragging:scrollView willDecelerate:decelerate];
    }
}

- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewWillBeginDecelerating:)]) {
        [_delegate xPageScrollViewWillBeginDecelerating:scrollView];
    }
}

- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewDidEndDecelerating:)]) {
        [_delegate xPageScrollViewDidEndDecelerating:scrollView];
    }
}

- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewDidEndScrollingAnimation:)]) {
        [_delegate xPageScrollViewDidEndScrollingAnimation:scrollView];
    }
}

- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xViewForZoomingInScrollView:)]) {
...
- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewWillBeginZooming:withView:)]) {
        [_delegate xPageScrollViewWillBeginZooming:scrollView withView:view];
    }
}

- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewDidEndZooming:withView:atScale:)]) {
        [_delegate xPageScrollViewDidEndZooming:scrollView withView:view atScale:scale];
    }
}

- (BOOL)scrollViewShouldScrollToTop:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewShouldScrollToTop:)]) {
...
- (void)scrollViewDidScrollToTop:(UIScrollView *)scrollView {
    if(_delegate != nil && [_delegate respondsToSelector:@selector(xPageScrollViewDidScrollToTop:)]) {
