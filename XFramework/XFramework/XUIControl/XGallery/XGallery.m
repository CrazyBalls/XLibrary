...
- (void)setPageIndex:(NSInteger)pageIndex {
    NSInteger index = 0;
    if ([XTool isArrayEmpty:imagesArr] == YES) {
        index = 0;
    } else {
...
- (void)addImages:(NSArray *)images {
    if ([XTool isArrayEmpty:images] == NO) {
        if (imagesArr == nil) {
...
- (NSInteger)numberOfSectionsInCollectionView:(UICollectionView *)collectionView {
    ...
    if ([XTool isArrayEmpty:imagesArr] == NO) {
        sectionCount = 1;
    }
    return sectionCount;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([XTool isArrayEmpty:imagesArr] == NO) {
        ...
        if ([unKnownObject isKindOfClass:[NSString class]] == YES) {
            ...
            if ([imagePath rangeOfString:@"http://"].location == NSNotFound) {
                if ([imagePath rangeOfString:@"/"].location == NSNotFound) {//不是地址,是文件名
                    if ([imagePath rangeOfString:@"."].location != NSNotFound) {//带后缀
                        ...
                        if ([type isEqualToString:@"gif"] == YES) {//gif
        ...
        } else if ([unKnownObject isKindOfClass:[NSData class]] == YES) {
            NSData *data = (NSData *)unKnownObject;
            [cell addImageForData:data];
        } else if ([unKnownObject isKindOfClass:[UIImage class]] == YES) {
